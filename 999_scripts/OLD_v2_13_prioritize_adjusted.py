...
def calculate_priority_score(row):
    ...
    Score = (Species × 100) + (IEDB × 30) - (Identity × 3.0) - (Coverage × 0.3)
    
    Higher is better.
    UPDATED: Stronger penalty for human identity to favor safer epitopes.
    """
    species_score = row['Num_Species'] * 100
    iedb_bonus = 30 if row['Has_IEDB'] else 0
    identity_penalty = row['Max_Human_Identity'] * 3.0  # INCREASED from 0.5
    coverage_penalty = row['Max_Human_Coverage'] * 0.3  # KEPT SAME
...
def prioritize_clusters(input_file, output_dir, log_dir):
    ...
    # Set up logging
    logger = Logger(log_dir, "v2_13_prioritize_adjusted")
    sys.stdout = logger
    
    print("="*80)
    print("EPITOPE CLUSTER PRIORITIZATION (ADJUSTED WEIGHTS)")
    print("="*80)
    ...
    print("\nPriority formula (UPDATED):")
    print("  Score = (Species × 100) + (IEDB × 30) - (Identity × 3.0) - (Coverage × 0.3)")
    print("  Where:")
    print("    - Species: Number of species (0-20)")
    print("    - IEDB: 30 if Has_IEDB, 0 otherwise")
    print("    - Identity: Max_Human_Identity (0-100%) - STRONG PENALTY")
    ...
    # Show top 50
    print("\n" + "="*80)
    print("TOP 50 EPITOPE CLUSTERS (ADJUSTED SCORING)")
    print("="*80)
    ...
    # Compare identity distribution: TIER1 vs all
    print("\n" + "="*80)
    print("HUMAN IDENTITY COMPARISON: TIER1 vs ALL")
    print("="*80)
    
    print(f"\nIdentity <80%:")
    print(f"  TIER1: {(tier1['Max_Human_Identity'] < 80).sum()}/{len(tier1)} ({(tier1['Max_Human_Identity'] < 80).sum()/len(tier1)*100:.1f}%)")
    print(f"  All:   {(clusters_filtered['Max_Human_Identity'] < 80).sum()}/{len(clusters_filtered)} ({(clusters_filtered['Max_Human_Identity'] < 80).sum()/len(clusters_filtered)*100:.1f}%)")
    
    print(f"\nIdentity <90%:")
    print(f"  TIER1: {(tier1['Max_Human_Identity'] < 90).sum()}/{len(tier1)} ({(tier1['Max_Human_Identity'] < 90).sum()/len(tier1)*100:.1f}%)")
    print(f"  All:   {(clusters_filtered['Max_Human_Identity'] < 90).sum()}/{len(clusters_filtered)} ({(clusters_filtered['Max_Human_Identity'] < 90).sum()/len(clusters_filtered)*100:.1f}%)")
