...
def filter_human_similarity_80percent(output_dir, log_dir):
    """
    Human filtering with 80% identity threshold.
    
    EXCLUDE if: identity >= 80% (regardless of coverage)
    PASS: identity < 80% (any coverage)
    """
    
    # Set up logging
    logger = Logger(log_dir, "v2_17_human_filter_80percent")
    sys.stdout = logger
    
    print("="*80)
    print("HUMAN PROTEOME SIMILARITY FILTER (80% IDENTITY THRESHOLD)")
    print("="*80)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    print("\nFiltering Criteria:")
    print("  EXCLUDE: identity >= 80% (regardless of coverage)")
    print("  PASS: identity < 80% (any coverage)")
    print("\nGoal: Balance between safety and diversity for manual curation")
    ...
    # Find epitopes to EXCLUDE (identity >= 80%)
    print("\n" + "="*80)
    print("IDENTIFYING HIGH HUMAN SIMILARITY")
    print("="*80)
    
    high_similarity = blast_df[blast_df['Percent_Identity'] >= 80]
    epitopes_to_exclude = set(high_similarity['Epitope_Sequence'].unique())
    
    print(f"\nEpitopes to EXCLUDE:")
    print(f"  identity >= 80% (any coverage)")
    ...
    # Fill NaN (no human hits) - these automatically PASS
    ...
    print(f"  PASS (<80% identity): {passed:,} ({passed/total*100:.1f}%)")
    print(f"  EXCLUDE (≥80% identity): {excluded:,} ({excluded/total*100:.1f}%)")
    ...
    if len(passed_with_hits) > 0:
        ...
        print(f"  Min identity: {passed_with_hits['Max_Human_Identity'].min():.1f}%")
        ...
        for threshold in [0, 20, 40, 60, 70, 75]:
            count = (passed_with_hits['Max_Human_Identity'] >= threshold).sum()
            pct = count / len(passed_with_hits) * 100
    ...
    all_results_file = f"{output_dir}/all_epitopes_human_filtered_80pct.txt"
    ...
    passed_file = f"{output_dir}/epitopes_PASSED_human_filter_80pct.txt"
    ...
    excluded_file = f"{output_dir}/epitopes_EXCLUDED_human_80pct.txt"
    excluded_df.to_csv(excluded_file, sep='\t', index=False)
    print(f"✓ Saved EXCLUDED: {excluded_file} ({len(excluded_df):,} epitopes)")
    
    print(f"\nEnd time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*80)
    print("80% IDENTITY FILTERING COMPLETE")
    print("="*80)
    print("\nNext steps:")
    print("  1. Cluster epitopes with safe representative selection")
    print("  2. Prioritize by species breadth + low identity")
    print("  3. Manual alignment and curation of top candidates")
    print("="*80)
...
if __name__ == "__main__":
    ...
    filter_human_similarity_80percent(output_dir, log_dir)