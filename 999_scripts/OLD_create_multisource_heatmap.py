...
def create_multi_source_heatmap(output_dir):
    """
    Heatmap with species ordered by Gram classification (Gram+ first, then Gram-).
    """
    
    print("="*70)
    print("CREATING MULTI-SOURCE EPITOPE × SPECIES HEATMAP")
    print("="*70)
    ...
    # Order species: Gram+ first (alphabetically), then Gram- (alphabetically)
    all_species = mapping_df['Parsed_Species'].dropna().unique()
    species_present_grampos = sorted([s for s in all_species if s in gram_positive])
    species_present_gramneg = sorted([s for s in all_species if s in gram_negative])
    species_list = species_present_grampos + species_present_gramneg
    
    print(f"\nGram-positive species: {species_present_grampos}")
    print(f"Gram-negative species: {species_present_gramneg}")
    ...
    ax_heatmap = plt.axes([0.21, 0.08, 0.51, 0.85])
    ax_metaproteome = plt.axes([0.73, 0.08, 0.02, 0.85])
    ax_method = plt.axes([0.76, 0.08, 0.04, 0.85])
    ax_fish = plt.axes([0.81, 0.08, 0.02, 0.85])
    ...
    gram_pos_count = len(species_present_grampos)
    if gram_pos_count > 0:
    ...
    ax_heatmap.set_xticklabels(species_list, rotation=45, ha='right', fontsize=10)
    ...
    # Title
    fig.text(0.5, 0.97, 'TOP50 Epitopes: Multi-Source Validation Across Species', 
    ...
    output_file = f"{output_dir}/heatmap_multisource_TOP50_final.png"
