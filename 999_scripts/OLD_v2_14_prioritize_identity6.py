...
def calculate_priority_score(row):
    ...
    Score = (Species × 100) + (IEDB × 30) - (Identity × 6.0) - (Coverage × 0.5)
    
    Higher is better.
    STRONG penalty for human identity to prioritize safer epitopes.
    """
    species_score = row['Num_Species'] * 100
    iedb_bonus = 30 if row['Has_IEDB'] else 0
    identity_penalty = row['Max_Human_Identity'] * 6.0  # STRONG PENALTY
    coverage_penalty = row['Max_Human_Coverage'] * 0.5
...
def prioritize_clusters(input_file, output_dir, log_dir):
    ...
    # Set up logging
    logger = Logger(log_dir, "v2_14_prioritize_identity6")
    sys.stdout = logger
    
    print("="*80)
    print("EPITOPE CLUSTER PRIORITIZATION (IDENTITY × 6.0)")
    print("="*80)
    ...
    print("\nPriority formula:")
    print("  Score = (Species × 100) + (IEDB × 30) - (Identity × 6.0) - (Coverage × 0.5)")
    print("  Where:")
    ...
    print("\n" + "="*80)
    print("CHECKING SPECIFIC SEQUENCE")
    print("="*80)
    ...
    if len(target_row) > 0:
        rank = (clusters_filtered['Representative_Sequence'] == target_seq).idxmax()
        position = clusters_filtered.index.get_loc(rank) + 1
        ...
        print(f"  Tier: {row['Tier'] if 'Tier' in row else 'Not assigned yet'}")
    else:
