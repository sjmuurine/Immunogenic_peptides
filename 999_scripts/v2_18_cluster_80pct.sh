...
#SBATCH --output=/scratch/project_2009813/JOHANNA/II_potential_epitopes/000_logs/cluster_80pct_%j.out
#SBATCH --error=/scratch/project_2009813/JOHANNA/II_potential_epitopes/000_logs/cluster_80pct_%j.err

echo "=================================================="
echo "CD-HIT Clustering (80% Identity Filtered Epitopes)"
echo "=================================================="
...
INPUT_FILE="${BASE_DIR}/003_result_comparisons/epitopes_PASSED_human_filter_80pct.txt"
OUTPUT_DIR="${BASE_DIR}/004_cluster"
WORK_DIR="${OUTPUT_DIR}/clustering_data"  # Better name than "temp"

mkdir -p ${OUTPUT_DIR}
mkdir -p ${WORK_DIR}

echo "Step 1: Preparing FASTA from 80% filtered epitopes"
echo "=================================================="

python3 << 'PREP_FASTA'
import pandas as pd

input_file = '/scratch/project_2009813/JOHANNA/II_potential_epitopes/003_result_comparisons/epitopes_PASSED_human_filter_80pct.txt'
output_fasta = '/scratch/project_2009813/JOHANNA/II_potential_epitopes/004_cluster/clustering_data/epitopes_80pct.fasta'
...
# Save IMPORTANT index mapping (includes source proteins for alignments!)
mapping_file = '/scratch/project_2009813/JOHANNA/II_potential_epitopes/004_cluster/clustering_data/epitope_index_mapping.txt'
...
print(f"✓ IMPORTANT: Index mapping saved: {mapping_file}")
print("  This file contains source proteins needed for alignments!")
...
FASTA_FILE="${WORK_DIR}/epitopes_80pct.fasta"
CDHIT_OUTPUT="${WORK_DIR}/epitopes_80pct_clustered"
...
def parse_species_from_protein_id(protein_id):
    """Parse species from protein ID - 20 SPECIES."""
...
cluster_file = '/scratch/project_2009813/JOHANNA/II_potential_epitopes/004_cluster/clustering_data/epitopes_80pct_clustered.clstr'
...
print("Priority: 1) Lowest human identity, 2) IEDB bonus, 3) Most proteins")

for cluster_id, members in clusters.items():
    if (cluster_id + 1) % 5000 == 0:
        print(f"  Processing cluster {cluster_id + 1:,}/{len(clusters):,}...", end='\r')
    
    member_indices = [m[0] for m in members]
    
    # SAFE REPRESENTATIVE SELECTION
    best_idx = None
    ...
    for idx in member_indices:
        ...
        identity = float(epitope.get('Max_Human_Identity', 100))
        has_iedb = 'IEDB' in str(epitope.get('Method', ''))
        num_proteins = int(epitope.get('Num_Source_Proteins', 0))
        
        # Selection logic: prioritize lowest identity
        is_better = False
    ...
    cluster_results.append({
        ...
        'Source_Proteins_All': '; '.join(all_proteins[:50])  # Limit for file size
    })

print()

# Create DataFrame and sort by species
cluster_df = pd.DataFrame(cluster_results)
cluster_df = cluster_df.sort_values('Num_Species', ascending=False)

# Save
output_file = '/scratch/project_2009813/JOHANNA/II_potential_epitopes/004_cluster/epitope_clusters_80pct_summary.txt'
...
print(f"\nHuman similarity in representatives (SHOULD BE MUCH BETTER!):")
...
for threshold in [60, 70, 75, 80]:
    count = (cluster_df['Max_Human_Identity'] >= threshold).sum()
...
for _, row in cluster_df.head(10).iterrows():
    print(f"  {row['Representative_Sequence'][:35]:35} - {row['Num_Species']:2}sp, {row['Max_Human_Identity']:5.1f}% ID")

PARSE_CLUSTERS

echo ""
echo "Step 4: Preserving clustering data"
echo "=================================="
echo "Keeping clustering_data directory (contains important files):"
echo "  - epitope_index_mapping.txt (source proteins for alignments)"
echo "  - CD-HIT cluster files"
echo "  - FASTA files"
...
echo ""
echo "Next steps:"
echo "  1. Filter low-complexity sequences"
echo "  2. Prioritize by species + low identity"
echo "  3. Extract protein regions for top candidates"
echo "  4. Manual alignment and selection"
echo "=================================================="