...
def calculate_priority_score(row):
    ...
    Score = (Species × 90) + (IEDB × 30) - (Identity × 10.0) - (Coverage × 0.5)
    
    Higher is better.
    VERY STRONG penalty for human identity - safety prioritized!
    """
    species_score = row['Num_Species'] * 90  # Reduced from 100
    iedb_bonus = 30 if row['Has_IEDB'] else 0
    identity_penalty = row['Max_Human_Identity'] * 10.0  # VERY STRONG PENALTY
