...
def analyze_epitope_sequences(input_file, output_file, similarity_threshold=95):
    """
    Groups BLAST results by epitope sequence similarity.
    
    Parameters:
    - input_file: epitope_blast_COMBINED_high_quality.txt
    - output_file: output with sequence clustering
    - similarity_threshold: group sequences with ≥ this % identity (default 95%)
    """
    ...
    for seq, group in sequence_groups:
        ...
        # Get representative protein matches
        ...
        anatomia_protein = top_anatomia['Protein_ID'].values[0] if len(top_anatomia) > 0 else "None"
        anatomia_identity = top_anatomia['Percent_Identity'].values[0] if len(top_anatomia) > 0 else 0
        
        literature_protein = top_literature['Protein_ID'].values[0] if len(top_literature) > 0 else "None"
    ...
    print(summary_df[['Epitope_Sequence', 'Total_Hits', 'ANATOMIA_Hits', 'LITERATURE_Hits']].head())
    print(f"\n{'='*60}")
...
input_file = '/scratch/project_2009813/JOHANNA/II_potential_epitopes/002_results/epitope_blast_COMBINED_high_quality.txt'
output_file = '/scratch/project_2009813/JOHANNA/II_potential_epitopes/002_results/epitope_sequences_summary.txt'
